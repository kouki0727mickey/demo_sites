...
export const schema=z.discriminatedUnion('kind',[
 z.object({kind:z.literal('text'),model:z.enum(['claude-opus-5-5','gpt-6-sol','gpt-6-luna']),apiKey:key,prompt:z.string().trim().min(1).max(12000),prefix:z.string().max(50000).default(''),cache:z.boolean().default(false)}),
...
export function buildText(input:Extract<z.infer<typeof schema>,{kind:'text'}>){
 const {model,prompt,prefix,cache,apiKey}=input;
 if(model==='claude-opus-5-5')return {url:'https://api.anthropic.com/v1/messages',headers:{'x-api-key':apiKey,'anthropic-version':'2023-06-01'},body:{model,max_tokens:4096,thinking:{type:'adaptive'},messages:[{role:'user',content:prompt}],...(prefix?{system:[{type:'text',text:prefix,...(cache?{cache_control:{type:'ephemeral'}}:{})}]}:{})}};
 return {url:'https://api.openai.com/v1/responses',headers:{Authorization:'Bearer '+apiKey},body:{model,store:false,max_output_tokens:4096,reasoning:{effort:'medium'},input:[...(prefix?[{role:'developer',content:[{type:'input_text',text:prefix,...(cache?{prompt_cache_breakpoint:{mode:'explicit'}}:{})}]}]:[]),{role:'user',content:prompt}],...(cache?{prompt_cache_options:{mode:'explicit'}}:{})}};
}
...
export async function execute(input:unknown,transport:typeof fetch=fetch){
 ...
 const config=buildText(p);let raw:any;
 try{const r=await transport(config.url,{method:'POST',headers:Object.fromEntries(Object.entries({...config.headers,'Content-Type':'application/json'}).filter((x):x is [string,string]=>typeof x[1]==='string')),body:JSON.stringify(config.body),redirect:'manual',signal:AbortSignal.timeout(90000)});
 ...
 raw=await r.json();}catch(e){if(e instanceof EvaluationError)throw e;throw new EvaluationError('接続できないか90秒を超えました。提供元で処理・課金が続く場合があります。',504);}
 ...
 return {kind:'text',model:p.model,text:text||'回答本文がありません。推論中に上限へ達した可能性があります。',seconds:(performance.now()-start)/1000,usage,usd,incomplete:anthropic?raw.stop_reason!=='end_turn':raw.status!=='completed'};
}
